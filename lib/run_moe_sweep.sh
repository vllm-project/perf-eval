#!/usr/bin/env bash
# MoE-backend sweep: run the full eval suite once per backend listed in
# vllm.moe_backends in the workload YAML.  Results land in
# results/<name>/moe-<backend>/ for each backend.
#
# Invoked by run.sh via exec when WORKLOAD_MOE_BACKENDS is non-empty.
# Can also be run directly: ./lib/run_moe_sweep.sh workloads/foo.yaml
#
# Usage: ./lib/run_moe_sweep.sh <workload.yaml>
...
mapfile -t MOE_BACKENDS <<< "$WORKLOAD_MOE_BACKENDS"

for MOE_BACKEND in "${MOE_BACKENDS[@]}"; do
  [[ -z "$MOE_BACKEND" ]] && continue

  if [[ "$MOE_BACKEND" == "default" ]]; then
    echo "=== :brain: moe backend: (vLLM default)"
    RESULTS_DIR="results/${WORKLOAD_NAME}/moe-default"
    EFFECTIVE_SERVE_ARGS="$WORKLOAD_SERVE_ARGS"
  else
    echo "=== :brain: moe backend: ${MOE_BACKEND}"
    RESULTS_DIR="results/${WORKLOAD_NAME}/moe-${MOE_BACKEND}"
    # --moe-backend is a vLLM server arg, not an env var.
    EFFECTIVE_SERVE_ARGS="${WORKLOAD_SERVE_ARGS} --moe-backend ${MOE_BACKEND}"
  fi
  mkdir -p "$RESULTS_DIR"

  export MOE_BACKEND

  CONTAINER="perf-eval-${WORKLOAD_NAME}-moe-${MOE_BACKEND}-$$"
  ...
  if [[ "$MOE_BACKEND" == "default" ]]; then
    echo "--- :mag: moe backend selected by vLLM:"
    _moe_line=""
    if [[ "${WORKLOAD_SERVER_RUNTIME:-docker}" == "native" ]]; then
      _moe_line=$(grep -oE "Using .+ MoE backend" \
        "${VLLM_LOG_FILE:-/dev/null}" 2>/dev/null | head -1) || true
    else
      _moe_line=$(docker logs "$CONTAINER" 2>&1 \
        | grep -oE "Using .+ MoE backend" | head -1) || true
    fi
    if [[ -n "$_moe_line" ]]; then
      echo "  $_moe_line"
      echo "$_moe_line" > "${RESULTS_DIR}/moe_backend.txt"
      # Extract everything between "Using " and " MoE backend"
      _resolved=$(echo "$_moe_line" | sed 's/^Using //; s/ MoE backend$//') || true
      [[ -n "$_resolved" ]] && export MOE_BACKEND="$_resolved"
    else
      echo "  (moe backend selection line not found in log)"
      echo "unknown" > "${RESULTS_DIR}/moe_backend.txt"
    fi
  else
    echo "$MOE_BACKEND" > "${RESULTS_DIR}/moe_backend.txt"
  fi
  ...
  while IFS=$'\t' read -r bname backend dataset isl osl nprompts conc speed_subset speed_category; do
    ...
    if ! run_vllm_bench "$CONTAINER" "$PORT" "$WORKLOAD_MODEL" \
      ...
      echo "^^^ +++ ERROR: run_vllm_bench failed for ${bname} (moe backend ${MOE_BACKEND}); skipping run" >&2
      continue
    fi
