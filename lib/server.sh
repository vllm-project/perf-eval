...
#
# Functions:
#   start_server <container> <port> <image> <model> <serve_args> <env>
#   wait_healthy <port> [timeout_s=1500]
#   stop_server  <container>
#
# `env` is a newline-separated list of KEY=VALUE pairs, each injected into
# the container with -e. As a special case, HF_HOME is also bind-mounted at
# the same path inside the container so the model cache on the host is
# visible to vLLM.
#
...
start_server() {
  ...
  local docker_args=(--gpus all --ipc=host -p "${port}:${port}")
  local hf_home=""
