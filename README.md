...
- **`bfcl.max_test_cases`** subsamples a category to the first N test cases (by BFCL id order) instead of running the full set — e.g. `multi_turn` (~800 cases) down to 100. Set a single integer to cap every category, or a map per category (`multi_turn: 100`). Override per-run with `BFCL_MAX_TEST_CASES`. Scores are partial-eval only and are not comparable to full BFCL leaderboard numbers.
