...
vllm_bench:              # perf runs (optional) — fed to the perf dashboard
  ...
  sweep:                 # optional Pareto sweep; expands into extra configs (see below)
    shapes:                             # fixed-sequence-length shapes, InferenceX-style
      - {isl: 8192, osl: 1024}
      - {isl: 1024, osl: 1024}
    concurrency: {start: 4, end: 128}   # x2 ladder 4,8,...,128 — or an explicit list
    prompts_per_concurrency: 4          # num_prompts = max(min_prompts, this x conc)
    min_prompts: 32
    repetitions: 1                      # optional; same rules as configs[].repetitions
...
- **`vllm.startup_timeout_s`** limits the readiness wait (default: `3600`). The wait ends early if the native server process exits or the Docker container stops or disappears. Server lifecycle regression tests run in the CPU pipeline-generation step.
...
- **`vllm_bench.sweep` fans a Pareto sweep out into ordinary bench configs.** Every `shapes[]` entry (`isl`/`osl`) is measured at every concurrency of the ladder, on the same server, using `dataset: random` with `backend: openai` (override with `backend:`). `concurrency` is either an explicit list or `{start, end, factor}` (default factor `2`): the ladder multiplies from `start` and always ends exactly at `end`, mirroring InferenceX's `conc-start`/`conc-end` search space. Each point gets its own config named `<name>-<isl>-in-<osl>-out` (default `name: sweep`, so runs land as `sweep-8k-in-1k-out-conc-32`), with `num_prompts = max(min_prompts, prompts_per_concurrency x conc)` and `num_warmups = conc` (one warmup wave) unless `args.num_warmups` is set explicitly. Rows ingest exactly like hand-written configs; the dashboard's `tput_per_gpu` vs `mean_intvty`/`median_intvty` (tok/s/user) columns, filtered by `isl`/`osl`, give the Pareto curve. Server-side sweeps (TP/EP/DP) are not expressed here yet — a workload is one server, so use one workload per parallelism config for now.
